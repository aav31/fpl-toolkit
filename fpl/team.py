...
class Team:
    ...
    def __post_init__(self):
        ...
        if not isinstance(self.gkps, frozenset) or not all(isinstance(player, Player) for player in self.gkps):
            raise TypeError("gkps must be a FrozenSet of Player instances")
        if not isinstance(self.defs, frozenset) or not all(isinstance(player, Player) for player in self.defs):
            raise TypeError("defs must be a FrozenSet of Player instances")
        if not isinstance(self.mids, frozenset) or not all(isinstance(player, Player) for player in self.mids):
            raise TypeError("mids must be a FrozenSet of Player instances")
        if not isinstance(self.fwds, frozenset) or not all(isinstance(player, Player) for player in self.fwds):
    ...
    def __eq__(self, other: "Team") -> bool:
        ...
        """
        if self.gkps != other.gkps: return False
        if self.defs != other.defs: return False
        if self.mids != other.mids: return False
        if self.fwds != other.fwds: return False
    ...
    def __str__(self):
        ...
        result += "{GKPS}\n"
        for player in sorted(list(self.gkps)): result += "{}\n".format(player)
        
        result += position_delimiter
        result += "{DEFS}\n"
        for player in sorted(list(self.defs)): result += "{}\n".format(player)
        
        result += position_delimiter
        result += "{MIDS}\n"
        for player in sorted(list(self.mids)): result += "{}\n".format(player)
        
        result += position_delimiter
        result += "{FWDS}\n"
        for player in sorted(list(self.fwds)): result += "{}\n".format(player)    
    ...
    def is_feasible(self) -> bool:
        ...
        for player in self.gkps: club_count[player.club] += 1
        for player in self.defs: club_count[player.club] += 1
        for player in self.mids: club_count[player.club] += 1
        for player in self.fwds: club_count[player.club] += 1
        if any(n_players > 3 for n_players in club_count.values()): return False
        
        if len(self.gkps) != 2: return False
        if len(self.defs) != 5: return False
        if len(self.mids) != 5: return False
        if len(self.fwds) != 3: return False
    ...
    def transfer_player(self, out_player: Player, in_player: Player) -> "Team":
        ...
        if out_player.position == 1:
            new_gkps = frozenset(player for player in self.gkps if player != out_player) | frozenset([in_player])
            return replace(self, gkps=new_gkps, money_in_bank=new_money_in_bank, free_transfers=new_free_transfers)
        elif out_player.position == 2:
            new_defs = frozenset(player for player in self.defs if player != out_player) | frozenset([in_player])
            return replace(self, defs=new_defs, money_in_bank=new_money_in_bank, free_transfers=new_free_transfers)
        elif out_player.position == 3:
            new_mids = frozenset(player for player in self.mids if player != out_player) | frozenset([in_player])
            return replace(self, mids=new_mids, money_in_bank=new_money_in_bank, free_transfers=new_free_transfers)
        elif out_player.position == 4:
            new_fwds = frozenset(player for player in self.fwds if player != out_player) | frozenset([in_player])
            return replace(self, fwds=new_fwds, money_in_bank=new_money_in_bank, free_transfers=new_free_transfers)
        else:
