...
class TestLoader(unittest.TestCase):
    def test_get_static_info(self):
        ...
        for p in static_info["elements"]:
            if p["web_name"] == "Palmer":
                self.assertEqual(182, p["id"], "Player unique identifier should not change")
    ...
    def test_get_fixtures(self):
        ...
        for fixture in fixtures:
            if (fixture["event"] == 4) and (fixture["team_h"] == 18):
                self.assertEqual(fixture["team_a"], 1, "Spurs play Arsenal in gameweek 4")
                self.assertEqual(fixture["kickoff_time"], "2024-09-15T13:00:00Z", "The game occurs at 13:00 UTC")
    ...
    def test_get_fixture_info(self):
        ...
        self.assertEqual(fixture_info["kickoff_time"], "2024-09-15T13:00:00Z", "The game occurs at 13:00 UTC")
    ...
    def test_get_fixtures_for_gameweek(self):
        ...
        def is_chelsea_man_city(x): return (x["team_h"] == 6) and (x["team_a"] == 13)
        def is_man_utd_fulham(x): return (x["team_h"] == 14) and (x["team_a"] == 9)
    ...
    def test_get_my_team_from_api(self):
        result = Loader.get_my_team_from_api("aledvaghela@gmail.com", "NCP@jxy7pau-bvm3cuf", 3247546)
    ...
    def test_get_next_gameweek(self):
        self.assertEqual(Loader.get_next_gameweek("2024-08-16 17:00:00"), 1, "Before deadline")
        self.assertEqual(Loader.get_next_gameweek("2024-08-16 21:00:00"), 2, 'After deadline')
        
    def test_get_my_historical_team_from_gameweek(self):
        print("Warning: test_get_my_historical_team_for_gameweek needs to be implemented")
        return
    ...
    def test_get_player_historical_info_for_gameweek(self):
        print("Warning: test_get_player_historical_info_for_gameweek needs to be implemented")
        return
    ...
    def test_get_player_future_info_for_gameweek(self):
        ...
        self.assertEqual(len(player_future_info), 1, "Chelsea have one game in the final gameweek")
        self.assertEqual(player_future_info[0]["team_a"], 6, "Chelsea (away) play Nottingham Forest (home) on the final day")
