...
class TestOptimizer(unittest.TestCase):
    def setUp(self):
        ...
        self.team = Team(
            ...
            defs=frozenset([self.def_club_1, self.def_club_2, self.def_club_3, self.def_club_4, self.def_club_5]),
            mids=frozenset([self.mid_club_1, self.mid_club_2, self.mid_club_3, self.mid_club_4, self.mid_club_5]),
    ...
    def test_optimal_formation(self):
        ...
        self.assertEqual(result["defs"], set([self.pm[3], self.pm[4], self.pm[5], self.pm[6]]))
        self.assertEqual(result["mids"], set([self.pm[8], self.pm[9], self.pm[10], self.pm[11]]))
        ...
        self.assertEqual(result["mids"], set([self.pm[9], self.pm[10], self.pm[11], self.pm[12]]))
        ...
        result = Optimizer.optimal_formation(self.team, MockCalculator, 0)
        self.assertEqual(result["gkps"], set([self.pm[1]]))
        self.assertEqual(result["defs"], set([self.pm[3], self.pm[4], self.pm[5], self.pm[6], self.pm[7]]))
        self.assertEqual(result["mids"], set([self.pm[8], self.pm[9], self.pm[10], self.pm[11]]))
    ...
    def test_discounted_reward_simple(self):
        class MockCalculator(ExpectedPointsCalculator):
            def get_expected_points(player_id: int, gameweek: int) -> float:
                if gameweek == 1: return 10
                if gameweek == 2: return 20
                if gameweek == 3: return 30
        ...
        actual = Optimizer.discounted_reward(team, MockCalculator, gameweek=1, horizon=3)
        ...
        actual = Optimizer.discounted_reward(team, MockCalculator, gameweek=1, horizon=1)
        expected = 12 * (10) - 3.2
        self.assertAlmostEqual(actual, expected, msg="Penalty applied but only one week horizon")

    def test_discounted_reward_complex(self):
        class MockCalculator(ExpectedPointsCalculator):
            def get_expected_points(player_id: int, gameweek: int) -> float:
                if gameweek == 1: return self.epm_B[player_id]
                if gameweek == 2: return self.epm_C[player_id]
        ...
        actual = Optimizer.discounted_reward(team, MockCalculator, gameweek=1, horizon=2, gamma=0.5)
    ...
    def test_optimize_team_1(self):
        # no candidates
        candidates = []
        top_three = Optimizer.optimize_team(self.team, candidates, epc=self.epcZero, gameweek=1, horizon=1, max_transfers=1)
        self.assertEqual(len(top_three), 1)
        x_score, x_team = heapq.heappop(top_three)
        self.assertAlmostEqual(x_score, -2.4, "self.team has 1 free transfers corresponding to -2.4 points")
    ...
    def test_optimize_team_4(self):
        ...
        candidates = [
            Player(element=200, name='Doaky', position=4, club=20, cost=70)
        ]
        ...
        top_three = Optimizer.optimize_team(self.team, candidates, epc=MockCalculator, gameweek=1, horizon=1, max_transfers=3)
        self.assertEqual(len(top_three), 3)
    ...
    def test_optimize_team_5(self):
        ...
        top_three = Optimizer.optimize_team(self.team, candidates, epc=MockCalculator, gameweek=1, horizon=1, max_transfers=1)
        self.assertEqual(len(top_three), 3)
    ...
    def test_optimize_team_6(self):
        ...
        # two week horizon
        top_three = Optimizer.optimize_team(self.team, candidates, epc=MockCalculator, gameweek=1, horizon=2, max_transfers=1)
        self.assertEqual(len(top_three), 3)
    ...
    def test_optimize_team_7(self):
        ...
        # one week horizon
        top_three = Optimizer.optimize_team(self.team,
                                            candidates,
                                            epc=MockCalculator,
                                            gameweek=1,
                                            horizon=1,
        ...
        second_best_score, second_best_team = heapq.heappop(top_three)
        first_best_score, first_best_team = heapq.heappop(top_three)
        self.assertEqual(first_best_score, 3) # should be three from transferring in the two new players and wildcard
        self.assertEqual(first_best_team.money_in_bank, 20)
        self.assertEqual(first_best_team.free_transfers, -1)
        self.assertEqual(second_best_score, 3) # should be three from transferring in the two new players and wildcard
        self.assertEqual(second_best_team.money_in_bank, 15)
        self.assertEqual(second_best_team.free_transfers, -1)
        self.assertEqual(third_best_score, 3) # should be three from transferring in the two new players and wildcard
